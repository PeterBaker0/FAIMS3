...
export const APINotebookListSchema = z.object({
  // Backward compatible top-level name mirror from project.name
  name: z.string(),
  is_admin: z.boolean(),
  ...
  // Backward compatible team/status aliases
...
export const APINotebookGetSchema = z.object({
  project: ProjectInfoSchema,
  // Notebook specification payload
...
export const NotebookPayloadSchema = z.object({
  ...
  metadata: z.union([ProjectMetadataSchema, z.record(z.unknown())]),
});
export type NotebookPayload = z.infer<typeof NotebookPayloadSchema>;

export const NotebookEditableDetailsSchema = NotebookPayloadSchema;
export type NotebookEditableDetails = z.infer<
  typeof NotebookEditableDetailsSchema
>;

export const ProjectEditableDetailsSchema = ProjectInfoSchema.pick({
  name: true,
  description: true,
  teamId: true,
  templateId: true,
});
export type ProjectEditableDetails = z.infer<typeof ProjectEditableDetailsSchema>;

export const CreateNotebookFromTemplateSchema = z.object({
  project: ProjectEditableDetailsSchema.pick({
    name: true,
    description: true,
    teamId: true,
  }).extend({
...
export const PutUpdateNotebookInputSchema = z.object({
  project: ProjectEditableDetailsSchema.partial().optional(),
  notebook: NotebookPayloadSchema.optional(),
});
