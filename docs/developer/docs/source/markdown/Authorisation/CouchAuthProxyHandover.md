...
- [ ] New data DB init includes `_design/acl` with project-scoped `dbacl`.
- [ ] Every new rec/frev/avp/att from the engine has correct ACL fields.
- [ ] Integration test proves guest isolation + contributor access + parent
      inheritance after third-party edit.
- [ ] Conductor admin paths still work on internal URL.
- [ ] No migration code required for the proof (empty/new DB).
...
- [ ] Migration upgrades a fixture DB of unstamped docs to enforced ACL.
- [ ] Unit + integration + e2e coverage for guest/contributor matrix.
- [ ] Cutover runbook: backfill → flip public URL → rebuild client DBs.
- [ ] Lint/typecheck/tests green for touched packages.
...
```text
[ ] Read this handover + couch-auth-proxy USER-GUIDE.md (ACL sections)
[ ] Read PermissionModel.md + dataDB security.ts + permissions VDU
[ ] Skim database engine create/update/delete + attachment service
[ ] Phase A: helpers → stamp writes → initDataDB → compose proxy
[ ] Phase A: run unit + integration ACL proof (guest/contributor/parent)
[ ] Commit/push; do not start backfill until proof is green
[ ] Phase B: DATA migration; hardening env; e2e; docs
